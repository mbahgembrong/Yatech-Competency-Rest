...
const checkUsername = (req) => {
    return new Promise((resolve, reject) => {
        Users.findOne({ where: { username: req.body.username } }).then((emp) => {
            if (emp) {
                resolve(true)
            } else {
                resolve(false)
...
const checkEmail = (req) => {
    return new Promise((resolve, reject) => {
        Users.findOne({ where: { email: req.body.email } }).then((emp) => {
            if (emp) {
                resolve(true)
            } else {
                resolve(false)
...
module.exports = {
    register, authentication, logout, checkUsername, checkEmail
}